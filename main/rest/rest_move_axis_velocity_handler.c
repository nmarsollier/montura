/* Rest - rest_move_axis_velocity_handler.c
 *
 * Purpose: accept continuous axis velocity commands for manual slewing.
 *
...
esp_err_t rest_move_axis_velocity_handler(httpd_req_t *request) {
    ...
    MountResult result = mount_set_move_axis_velocity(ra_rate, dec_rate);
